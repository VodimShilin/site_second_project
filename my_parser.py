...
def open_new_window(current_section, source):
    with webdriver.Chrome(
        ...
        for i in range(300):
         ...
         #current_ = dr.find_elements_by_class_name('problem_container')
        flag = True
        while flag:
            dr.implicitly_wait(5900)
            dr.find_element_by_tag_name('body').send_keys(
                webdriver.common.keys.Keys.END)
            dr.implicitly_wait(5900)
            next_ = dr.find_elements_by_class_name('problem_container')
            if next_ == current_:
                flag = False
        #    current_ = next_
...
def get_items_from_site(section: str = 'math'):
        ...
        try:
            ...
            if index == 7:
                for ind, item in enumerate(
                    ...
                    current_section = open_new_window(current_section, source)
